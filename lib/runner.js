...
module.exports = (locations = [], formatFn = require('pg-format'), isolationLevel) => {
  ...
  const _prepareDelete = (queries, schema, table, toDelete, where, options) => {
    const sqlZip = (separator, obj) => {
      const ks = R.keys(obj);
      const vs = R.values(obj);
      const aEqB = (aa, bb) => {
    ...
    const tableClause = tableSql(schema, table);
    const deleteClause = sqlZip(',', toDelete);
    const whereClause = R.when(Boolean, R.concat('WHERE '), sqlZip(' AND ', where));
    const queryName = options.returning ? '_delete_returning' : '_delete';
    return _prepare(sqlFormatter, queries, queryName, [tableClause, deleteClause, whereClause]);
  };
  ...
  const _delete = (pool, queries, schema) => (table, toDelete, where = {}, options = {}) => {
    const prepared = _prepareDelete(queries, schema, table, toDelete, where, options);
    debug(`Deleting ${prepared}`);
    return pool.query(...prepared);
  };

  const createPoolOfOne = (client) => ({
  ...
  const _withTransaction = (pool, makeRunner, schema) => (fn, customIsolationLevel) =>
    Promise.using(pool.getConnection(), (client) => Promise.try(() => {
      const poolOfOne = createPoolOfOne(client);
  ...
  const _withConnection = (pool, makeRunner, schema) => (fn) =>
    Promise.using(pool.getConnection(), (client) => Promise.try(() => {
      const poolOfOne = createPoolOfOne(client);
      const runner = makeRunner(poolOfOne).schema(schema);
      return fn(runner);
